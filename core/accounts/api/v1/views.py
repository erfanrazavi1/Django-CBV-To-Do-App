...
class RegistrationApiView(generics.GenericAPIView):
    """
    This API view handles the user registration process. It validates the input data using a serializer,
    saves the user, generates authentication tokens (refresh and access), and sends an email with the token.
    """
...
class CustomAuthToken(ObtainAuthToken):
    """
    This API view handles user authentication. It generates an authentication token for the user 
...
class CustomDiscardAuthToken(APIView):
    """
    This API view allows the user to log out by deleting their authentication token.
...
class ChangePasswordApiView(generics.GenericAPIView):
    """
    This API view allows an authenticated user to change their password. It validates the old password 
...
class ResetPasswordApiView(generics.GenericAPIView):
    """
    This API view allows an authenticated user to initiate the password reset process by sending
    ...
    def post(self, request, *args, **kwargs):
        ...
        email_obj = EmailMessage('email/reset-pass.tpl', {'token': token}, 'erfan6235@gmail.com', to=[email])
...
# API for confirming the password reset and applying the new password
class ResetPasswordConfirmApiView(generics.GenericAPIView):
    """
    This API view handles the password reset confirmation by validating the provided token, 
    decoding it to retrieve the user's ID, and then allowing the user to set a new password.
    """
